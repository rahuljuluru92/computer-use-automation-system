...
import { DISCOVERY_TOOLS, ToolRunner } from '../../src/discovery/tools.ts';
...
const EVIDENCE_ROOT = 'evidence/_scratch/tools-test';
...
class FakeSurface implements Surface {
  ...
  async navigate(target: string): Promise<void> { this.calls.push({ method: 'navigate', args: [target] }); }
...
function build(policy: Partial<PolicyConfig> = {}, params: Record<string, unknown> = {}, secrets: string[] = []) {
  surface = new FakeSurface(snap('member-detail'));
  const evidence = new EvidenceWriter({
    runId: `tools-${Math.random().toString(36).slice(2, 8)}`,
    root: EVIDENCE_ROOT,
  ...
  runner = new ToolRunner({ executor, surface, evidence, params, secretParams: secrets });
}
